...
function buildCiFixComment({ headSha, failedChecks, failureSummaries }) {
  ...
  parts.push(
    ...
    '1. **Legitimate feature change** — the PR intent is correct but implementation or tests need updating. Preserve product guarantees; update tests only when behaviour intentionally changed and document why.',
    ...
    `- ${config.finalClaudeScope}`,
    '- Tests must continue to guard backward compatibility — passing CI by lowering test standards is not acceptable.',
    ...
    '4. Push to this branch and comment: files changed, review verdict, and why tests still protect product behaviour.',
  );
...
async function processWorkflowRunFailure(github, owner, repo, workflowRun, core) {
  ...
  const allowed = config.ciFailureWorkflowRuns;
