...
const productSample = `${config.productPathPrefixes[0]}a/b.py`;
const repoShort = config.repoFullName.split('/')[1];
...
assert('ci-only label not exempt mixed changes', mg.sensitivePathReasons(
  [{ filename: '.github/workflows/foo.yml' }, { filename: productSample }],
  [mg.WORKFLOW_ONLY_LABEL]
).length === 1);
...
assert('product code with tests ok', mg.missingTestsReason([
  { filename: productSample, additions: 3 },
...
assert('fork sync PR link rejected', !mg.isInternalPullRequestLink(
  { number: 21, base: { repo: { full_name: `Milkmange/${repoShort}` } } },
  'MervinPraison',
  repoShort
));
