...
function reviewChainStarted(comments) {
  if (chain.chainKickPosted(comments)) return true;
  return comments.some((c) => config.reviewBotLogins.includes(c.user?.login));
}

function deriveStage(comments, evalResult) {
  if (evalResult.ready) return 'pipeline/merge-ready';
  if (!reviewChainStarted(comments)) return 'pipeline/reviews-pending';
